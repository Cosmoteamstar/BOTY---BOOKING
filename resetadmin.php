...
$SQL = "SELECT * FROM admins WHERE idAdmin=".$_SESSION["userIDA"]." AND pw=".$_POST["txtpw"];
$rsw = doSelect($SQL);
$reset = mysqli_fetch_assoc($rsw);
if($reset){
    if(isset($_POST["txtpaw"])){
    $SQL = "UPDATE admins SET pw=\"".$_POST["txtpaw"]."\"
    WHERE idAdmin=\"".$_SESSION["userIDA"]."\" ";
        doSQL($SQL,"อัพเดทข้อมูลสำเร็จ","กรุณากรอกรหัสผ่านเก่าให้ถูกต้อง","indexadmin.php");
    }else{
